 ...
 */
@WebServlet("/views/admin/adminReservationList")
...
//		System.out.println("cPage@list="+cPage);
		...
		//페이징바영역처리
		int totalContent = new AdminService().selectTotalResvContent();
...
//		System.out.printf("totalContent=%s, totalPage=%s%n", totalContent, totalPage);
		...
		//1.이전
		if(pageNo != 1) {
			pageBar += "<a href='"+request.getContextPath()+"/views/admin/adminReservationList?cPage="+(pageNo-1)+"'>[이전]</a>\n";
		}
		...
		while(pageNo<=pageEnd && pageNo<=totalPage) {
			...
			if(cPage == pageNo) {
				...
				pageBar += "<a href='"+request.getContextPath()+"/views/admin/adminReservationList?cPage="+pageNo+"'>"+pageNo+"</a>\n";				
			}
		...
		//3.다음
		if(pageNo <= totalPage) {
			pageBar += "<a href='"+request.getContextPath()+"/views/admin/adminReservationList?cPage="+pageNo+"'>[다음]</a>\n";							
		}
		...
		//4.view단처리
		request.getRequestDispatcher("/WEB-INF/views/admin/adminReservationList.jsp")
